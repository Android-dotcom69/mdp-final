// src/components/WebcamFeed.js - Backend API version
...
const WebcamFeed = ({ onFaceDetected, isActive = true }) => {
  ...
  const startDetection = () => {
    ...
    // Send a frame every 500ms (2 FPS) - balanced for backend latency
  ...
  const processFrame = useCallback(async () => {
    // Skip if already processing a frame (prevents request pile-up)
    ...
    try {
      // Capture screenshot as data URL, convert to blob
      ...
      // Send to backend for face detection + recognition
      ...
      if (response.faces && response.faces.length > 0) {
        ...
        // Map results to the format DashboardPage expects
      ...
      // Don't flood console on transient errors
